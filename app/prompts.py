...
1. **查物流** (query_logistics)：客户提供了物流单号/快递单号/订单号，希望查询对应物流
2. **查发货进度** (query_shipping_progress)：客户在问“发货了没”“到哪里了”“什么时候发”等发货或物流进度，但没有提供单号
3. **查库存** (query_inventory)：客户想查询商品是否有货/库存/现货
4. **获取报价单** (get_quote)：客户明确想获取报价单/报价表/报价清单
5. **其他** (not_sure_intent)：无法明确识别为以上意图

## 处理规则

### 泛化判断
- 不要机械依赖关键词匹配，要结合上下文理解客户真实诉求
- 如果客户表达的是没见过的新说法，但语义与某个已知意图高度一致，可以做合理但精准的推断，并归类到最匹配的意图
- 只有在无法稳定判断客户诉求归属时，才返回 not_sure_intent

### 查物流
- 识别到查物流意图后，从消息中提取物流单号/快递单号/订单号
- 只有在消息里能明确拿到单号时，才返回 query_logistics
- 返回 status: "success" 和 order_no

### 查发货进度
- 客户明确在问发货状态、物流进度、货到哪里了、什么时候发，但消息里没有提供单号时，返回 query_shipping_progress
- 返回 status: "success"
...
### 获取报价单
- 客户明确想看产品资料、产品清单、产品列表、产品图册、报价单、报价表、报价清单时，都识别为 get_quote
- 例如：“发一份产品表”“发一份产品清单”“看看有哪些产品”“发下产品图册” 都属于 get_quote
...
- 如果识别到多个意图，固定按 query_logistics、query_shipping_progress、query_inventory、get_quote 的顺序返回
...
查发货进度：[{"intent": "query_shipping_progress", "status": "success"}]
...
查发货进度+报价单：[{"intent": "query_shipping_progress", "status": "success"}, {"intent": "get_quote", "status": "success"}]
